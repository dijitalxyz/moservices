...
print("UserInput=" + userInput);
	...
	else if (userInput == "<?= getRssCommand('play') ?>" || userInput == "<?= getRssCommand('forward') ?>")
	{
		...
		{
			s = savedChannel;
			playItemURL("<?= getMosUrl().'?page=peerstv_get' ?>&amp;cid=" + curChannel, 0);
		}
		else
		{
			s = doModalRss( "<?= getMosUrl().'?page=rss_peerstv_player' ?>" + "&amp;cid=" + curChannel );
		}
	...
	{
	        url = "<?= getMosUrl().'?page=rss_peerstv_sets' ?>";
  ...
  <onRefresh>
	...
	if( channelsCount == 0 )
	{
		...
		url = "<?= getMosUrl().'?page=peerstv_channels' ?>";
	...
	/* get epg list */
	url = "<?= getMosUrl().'?page=peerstv_list' ?>";
	if( curChannel != "" ) url += "&amp;channel=" + curChannel;
	if( curDate    != "" ) url += "&amp;date="    + curDate;
	...
	if( itemsCount == 0 )
	{
		msgText = "<?= getMsg('peerstvNothingFound') ?>";
		setFocusItemIndex( 0 );
	}
	else
	{
...
<?php
	$s = getPeerstvConfigParameter('vfd');
	if( $s == 'mele' )
	{
?>
