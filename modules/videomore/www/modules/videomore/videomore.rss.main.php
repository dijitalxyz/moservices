...
class rssVideomoreView extends rssSkinHTile
{
	const itemWidth		= 290;
...
<?php
		$px = 0;
		$py = round( 583.5 / static::viewAreaHeight * 100, 4);
		$pw = 100;
...
<?php
		...
		$py = round( 590.5 / static::viewAreaHeight * 100, 4);
...
<?php
		$px = round( 780.5 / static::viewAreaWidth  * 100, 4);
		$py = round( 590.5 / static::viewAreaHeight * 100, 4);
		$pw = round( 480.5 / static::viewAreaWidth  * 100, 4);
		$ph = round(  32.5 / static::viewAreaHeight * 100, 4);
?>
    <text offsetXPC="<?= $px ?>" offsetYPC="<?= $py ?>" widthPC="<?= $pw ?>" heightPC="<?= $ph ?>"
     redraw="yes" align="right" lines="1" fontSize="15"
     backgroundColor="<?= static::itemUnFocusBgColor ?>"
...
<?php
		$px = round(  100.5 / static::viewAreaWidth  * 100, 4);
		$py = round(  622.5 / static::viewAreaHeight * 100, 4);
		$pw = round( 1160.5 / static::viewAreaWidth  * 100, 4);
		$ph = round(   78.5 / static::viewAreaHeight * 100, 4);

?>
    <text offsetXPC="<?= $px ?>" offsetYPC="<?= $py ?>" widthPC="<?= $pw ?>" heightPC="<?= $ph ?>"
     redraw="yes" align="left" lines="2" fontSize="14"
     backgroundColor="<?= static::itemUnFocusBgColor ?>"
...
?>
	...
	else if (userInput == "<?= getRssCommand('enter') ?>")
	{
		...
		url = "<?= getMosUrl().'?page=videomore_deyst' ?>" + "&amp;id=" + id;
		deyst = getUrl(url);
		if (deyst == 0) 
		{
			url = "<?= getMosUrl().'?page=videomore_list' ?>" + "&amp;p=1&amp;type=film&amp;query=" + id;
			moUrl = url;
			setRefreshTime(100);
			ret = "true";
		...
		{
			url = "<?= getMosUrl().'?page=videomore_get' ?>" + "&amp;id=" + id;
			url = getUrl(url);
			showIdle();
			playItemURL( url, 0 );
			cancelIdle();
	...
	{
	        url = "<?= getMosUrl().'?page=rss_videomore_menu' ?>";
	...
	else if (userInput == "<?= getRssCommand('home') ?>" )
	{
			url = "<?= getMosUrl().'?page=videomore_back' ?>";
			url = getUrl(url);
		if(( url != null )&amp;&amp;( url != "" ))
		{
			moUrl = "<?= getMosUrl().'?page=videomore_list' ?>" + "&amp;p=1&amp;type=film&amp;query=" + url;
			ret = "true";
			setRefreshTime(100);
		}
	}
	else if (userInput == "<?= getRssCommand('play') ?>")
	{
		...
		{
			showIdle();
			url = "<?= getMosUrl().'?page=videomore_plist'?>" + "&amp;id=" + id;
			writeStringToFile("/tmp/path.dat", url);
			url = doModalRss( url );
			cancelIdle();
			ret = "false";
		} 
...
?>
  ...
  <onEnter>
	moUrl = "<?= getMosUrl().'?page=videomore_list' ?>";
	savedItem = 0;
	movieTitle = "";
	movieDesc  = "";
	movieGenre = "";
