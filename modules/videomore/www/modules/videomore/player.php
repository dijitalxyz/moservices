	...
	<onEnter>
		...
		if (dlok != null || dlok != 0) {
			...
			if (iSize &gt; 0) {
				itemCount = 0;
				startItem = 0;
				itemsStart = getItemInfoarray("start");
				itemsStart = getStringArrayAt(itemsStart, 0);
				itemTitleArray = getItemInfoarray("title");
				itemURLArray  = getItemInfoarray("link");
				itemACTArray = getItemInfoarray("act");
				while(1) {
					itemURL  = getStringArrayAt(itemURLArray, itemCount);
					if ( itemURL == itemsStart) startItem = itemCount;
					itemCount += 1; itemSize = itemCount; 
					if(itemCount == iSize) break;
				}
			}
		}	
		if(itemSize == 0)
			...
			track_time = null;
			pauseVideo = -1;
			currentPlay = startItem;
			currentPlay = currentPlay-1;
			setRefreshTime(100);
		}
		a = itemsStart + " " + startItem;
		writeStringToFile("/tmp/start.dat", a);
		writeStringToFile("/tmp/_url.dat",itemURLArray);
		player_action = null;
	</onEnter>
...
<onRefresh>
	...
	if(playStatus == 0) {
		...
		if (currentPlay &gt;= itemSize) {
			...
			counterInfo = 5;			
			act = getStringArrayAt(itemACTArray, currentPlay);
...
?>
  ...
  >
	...
	<progressBar backgroundColor=0:0:0 offsetXPC=0 offsetYPC=85 widthPC=100 heightPC=15>
		...
		<text redraw=yes align=center offsetXPC=10 offsetYPC=5 widthPC=80 heightPC=40 fontSize=15 backgroundColor=-1:-1:-1 foregroundColor=200:200:0>
