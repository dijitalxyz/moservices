...
Transmission.prototype =
{
	...
	updateTurtleButton: function() {
		...
		if ( this[Prefs._TurtleState] ) {
			...
			t = [ 'Нажмите для отключения ограничения скорости' ];
		} else {
			w.removeClass('turtleEnabled');
			w.addClass('turtleDisabled');
			t = [ 'Нажмите для включения ограничения скорости' ];
		}
		t.push( '(', Transmission.fmt.speed(this._prefs[RPC._TurtleUpSpeedLimit]), 'отдача,',
		             Transmission.fmt.speed(this._prefs[RPC._TurtleDownSpeedLimit]), 'загрузка)' );
		w.attr( 'title', t.join(' ') );
	},
	...
	updatePrefs: function( prefs )
	{
		...
		if (!iPhone)
		{
			setInnerHTML( $('#limited_download_rate')[0], [ 'Ограничение (', Transmission.fmt.speed(dn_limit_k), ')' ].join('') );
			var key = dn_limited ? '#limited_download_rate'
			                       : '#unlimited_download_rate';
			$(key).deselectMenuSiblings().selectMenuItem();

			setInnerHTML( $('#limited_upload_rate')[0], [ 'Ограничение (', Transmission.fmt.speed(up_limit_k), ')' ].join('') );
	...
	updatePortStatus: function( status ){
		if(status['port-is-open'])
			$('#port_test').text('Порт открыт');
		else
			$('#port_test').text('Порт закрыт');
	},
	...
	updateInspector: function()
	{
		...
		{
			setInnerHTML( tab.name, 'Не Выбрано' );
		...
		name = torrents.length == 1
			? torrents[0].name()
			: torrents.length+' Выбранные задания';
		...
		var private_string = '';
		var fmt = Transmission.fmt;
		if( have_private && have_public ) private_string = 'Смешанный';
		else if( have_private ) private_string = 'Приватный торрент';
		else if( have_public ) private_string = 'Публичный торрент';
		...
		if( $(this._inspector_peers_list).is(':visible') ) {
			for( var k=0, torrent; torrent=torrents[k]; ++k ) {
				...
				html.push( '<table class="peer_list">',
				           ...
				           '<th class="addressCol">IP адрес</th>',
				...
				for( var i=0, tier; tier=torrent._trackerStats[i]; ++i ) {
					...
					for( var j=0, tracker; tracker=tier[j]; ++j ) {
						...
						html.push( '<li class="inspector_tracker_entry ', parity, '"><div class="tracker_host" title="', tracker.announce, '">',
						           ...
						           '<tr><th>Отдающих:</th><td>', (tracker.seederCount > -1 ? tracker.seederCount : na), '</td></tr>',
						           '<tr><th>Личеров:</th><td>', (tracker.leecherCount > -1 ? tracker.leecherCount : na), '</td></tr>',
						           '<tr><th>Загрузивших:</th><td>', (tracker.downloadCount > -1 ? tracker.downloadCount : na), '</td></tr>',
						           '</table></li>' );
					}
					html.push( '</ul>' );
				}
	...
	lastAnnounceStatus: function(tracker){
		var lastAnnounceLabel = 'Последнее обновление';
		...
		if (tracker.hasAnnounced) {
			...
			if (tracker.lastAnnounceSucceeded) {
				...
				lastAnnounceLabel = 'Ошибка обновления';
	...
	announceState: function(tracker){
		...
		switch (tracker.announceState) {
			case Torrent._TrackerActive:
				announceState = 'Обновление в процессе';
				break;
			case Torrent._TrackerWaiting:
				...
				announceState = 'Следующее обновление в ' + Transmission.fmt.timeInterval(timeUntilAnnounce);
				break;
			case Torrent._TrackerQueued:
				announceState = 'Обновление в очереди';
				break;
			case Torrent._TrackerInactive:
				announceState = tracker.isBackup ?
					'Трекер будет использоваться в качестве резервного' :
					'Обновление не запланировано';
				break;
			default:
				announceState = 'состояние обновления неизвестно: ' + tracker.announceState;
		}
		return announceState;
	},

	lastScrapeStatus: function(tracker){
		var lastScrapeLabel = 'Последний Scrape запрос';
		var lastScrape = 'Н/Д';
		if (tracker.hasScraped) {
			...
			if (tracker.lastScrapeSucceeded) {
				...
				lastScrapeLabel = 'Ошибка Scrape запроса';
	...
	showInspector: function() {
		...
		setInnerHTML( $('ul li#context_toggle_inspector')[0], 'Спрятать Инспектор' );
	...
	hideInspector: function() {
		...
		if (iPhone) {
			...
			setInnerHTML( $('ul li#context_toggle_inspector')[0], 'Показать Инспектор' );
		}
	...
	updateStatusbar: function()
	{
		...
		if( torrentCount == visibleCount )
			s = torrentCount + ' Заданий';
		else
			s = visibleCount + ' of ' + torrentCount + ' Заданий';
	...
	promptToRemoveTorrents:function( torrents )
	{
		if( torrents.length == 1 )
		{
			...
			var header = 'Удалить ' + torrent.name() + '?';
			var message = 'Торрент файл будет удален. Вы точно хотите этого?';
			dialog.confirm( header, message, 'Удалить', 'transmission.removeTorrents', torrents );
		}
		else
		{
			var header = 'Удалить ' + torrents.length + ' и отданное?';
			var message = 'Торрент файл и отданное будет удалено. Вы точно хотите этого?';
			dialog.confirm( header, message, 'Удалить', 'transmission.removeTorrents', torrents );
		}
	},
	...
	promptToRemoveTorrentsAndData:function( torrents )
	{
		...
		{
			var torrent = torrents[0],
				header = 'Удалить ' + torrent.name() + ' и загруженное?',
				message = 'Все загруженное будет удалено. Вы точно хотите этого?';
			dialog.confirm( header, message, 'Удалить', 'transmission.removeTorrentsAndData', torrents );
		}
		else
		{
			var header = 'Удалить ' + torrents.length + ' отданное и загруженное?',
				message = 'Все отданное и загруженное будет удалено. Вы точно хотите этого?';
			dialog.confirm( header, message, 'Удалить', 'transmission.removeTorrentsAndData', torrents );
		}
	},
