...
{
    if (location.match(/^udp:\/\//))
    {
        var exturl = new Object();
        exturl.mimetype = 'video/x-msvideo';
        exturl.objectType = OBJECT_TYPE_ITEM_EXTERNAL_URL;
        if (location.match(/^udp:\/\/@*(.+)\:(.+)$/))
        {
            exturl.location = "http://127.0.0.1:8080/udp/" + RegExp.$1 + ":" + RegExp.$2;
	}
	else
        if (location.match(/^udp:\/\/@*(.+)$/))
        {
            exturl.location = "http://127.0.0.1:8080/udp/" + RegExp.$1 + ":1234";
	}
	else
	{
        ...
        exturl.upnpclass = "object.item.videoItem"; //UPNP_CLASS_ITEM_MUSIC_TRACK;
        exturl.description = "Item from " + playlist.title;
