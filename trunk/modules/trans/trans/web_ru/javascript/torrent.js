...
Torrent.prototype =
{
	...
	stateStr: function() {
		switch( this.state() ) {
			case Torrent._StatusSeeding:        return 'Раздается';
			case Torrent._StatusDownloading:    return 'Загружается';
			case Torrent._StatusPaused:         return this.isFinished() ? 'Готово' : 'Приостановлен';
			case Torrent._StatusChecking:       return 'Проверка данных';
	...
	getPeerDetails: function()
	{
		...
		switch( st )
		{
			...
			case Torrent._StatusDownloading:
				var a = [ ];
				if(!compact_mode)
					a.push( 'Загрузка', this.peersSendingToUs(), 'из', this._peers_connected, 'пиров', '-' );
			...
			case Torrent._StatusSeeding:
				if(compact_mode){
					...
					// 'Seeding to 13 of 22 peers - UL: 36.2 Кб/с'
					c = [ 'Раздача', this.peersGettingFromUs(), 'из', this._peers_connected, 'пиров', '-', this.formatUL() ].join(' ');
				}
				break;

			case Torrent._StatusChecking:
				// 'Verifying local data (40% tested)'
				c = [ 'Проверка данных (', Transmission.fmt.percentString( 100.0 * this._recheckProgress ), '% проверено)' ].join('');
				break;
		}
		return c;
	},
	...
	refreshHTML: function() {
		...
		else if( notDone )
		{
			...
			c = [ Transmission.fmt.size( this._sizeWhenDone - this._leftUntilDone ),
			      ' из ', Transmission.fmt.size( this._sizeWhenDone ),
			      ' (', this.getPercentDoneStr(), '%)' ];
			if( this.isActive( ) ) {
				c.push( ' - ' );
				if (this._eta < 0 || this._eta >= Torrent._InfiniteTimeRemaining )
					c.push( 'время завершения неизвестно' );
				else
					c.push( Transmission.fmt.timeInterval(this._eta) + ' осталось' );
			}
		...
		else
		{
			...
			else
			{
				...
				c = [ Transmission.fmt.size( this._sizeWhenDone ), ' из ', Transmission.fmt.size( this._size ),
				      ' (', Transmission.fmt.percentString( 100.0 * this._sizeWhenDone / this._size ), '%)' ];
			}

			// append UL stats: ', uploaded 8.59 GiB (Ratio: 12.3)'
			c.push( ', отдано ', Transmission.fmt.size( this._upload_total ),
			        ' (Ратио ', Transmission.fmt.ratioString( this._upload_ratio ), ')' );
