...
//
// ------------------------------------
function rss_onlinerec_player_content()
{
...
?>
	...
	else if( input == "<?= getRssCommand('up') ?>" || input == "<?= getRssCommand('down') ?>" )
	{
		...
		setEnv( "onlinerecChannelNumber", currentItem );
		setEnv( "onlinerecChannelCommand", input );

		s = doModalRss( "<?= getMosUrl().'?page=rss_onlinerec_channels' ?>" );
		if( s != null &amp;&amp; s != "" )
		{
			currentItem = s;
			startVideo = 1;
  ...
  <onRefresh>
	...
	if (startVideo == 1)
	{
		...
		if( itemCount != 0 )
		{
			...
			cUrl = "<?= getMosUrl().'?page=onlinerec_get' ?>&amp;cid=" + cId;
	...
	if( timerEpg == 0 )
	{
		s = getUrl( "<?= getMosUrl().'?page=onlinerec_get_epg' ?>&amp;cid=" + cId );
		if ( s != null &amp;&amp; s != "" )
		{
...
<?php
	if(( $s = getOnlinerecConfigParameter('vfd')) != 'none' )
	{
?>
	/* VFD time */
	s = getTimeDate();
