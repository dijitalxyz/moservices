...
class ua_player_const
{
	...
	const timeline_offsetYPC		= "81"; 
	const timeline_heightPC			= "10.5";
		...
		<onUserInput>
				<script>
					...
					if (input == "<?= $key_left ?>")
					{
							if( pitemCount != 1 )
								{
									if( idx_play == 0 ) idx_play = pitemCount - 1;
										else idx_play -= 1;
									startVideo = 1;
								}
						ret = "true";
					...
					if (input == "<?= $key_right ?>" )
					{
							if( pitemCount != 1 )
								{
									idx_play -= -1;
									if( idx_play == pitemCount ) idx_play = 0;
									startVideo = 1;
								}
						ret = "true";
					}
		...
		if (dlok != null)
			{
				...
				while( count != pitemCount )
					{
					plinkArray = pushBackStringArray( plinkArray, getStringArrayAt(dlok, c)); c += 3;			
